...
// A "thread" safe map of type any:Anything.
// To avoid lock bottlenecks this map is dived to several (SHARD_COUNT) map shards.
type ConcurrentMap[V any] []*ConcurrentMapShared[V]

// A "thread" safe any to anything map.
...
func New[V any]() ConcurrentMap[V] {
	...
	for i := 0; i < SHARD_COUNT; i++ {
		m[i] = &ConcurrentMapShared[V]{items: make(map[any]V)}
	}
	return m
}

// GetShard returns shard under given key
func (m ConcurrentMap[V]) GetShard(key any) *ConcurrentMapShared[V] {
	return m[uint(fnv32(key))%uint(SHARD_COUNT)]
}

func (m ConcurrentMap[V]) MSet(data map[any]V) {
...
func (m ConcurrentMap[V]) Set(key any, value V) {
	// Get map shard.
	shard := m.GetShard(key)
	shard.Lock()
	shard.items[key] = value
...
func (m ConcurrentMap[V]) Upsert(key any, value V, cb UpsertCb[V]) (res V) {
	shard := m.GetShard(key)
	shard.Lock()
...
func (m ConcurrentMap[V]) SetIfAbsent(key any, value V) bool {
	// Get map shard.
	shard := m.GetShard(key)
	shard.Lock()
...
func (m ConcurrentMap[V]) Get(key any) (V, bool) {
	// Get shard
	shard := m.GetShard(key)
	shard.RLock()
	// Get item from shard.
...
func (m ConcurrentMap[V]) Has(key any) bool {
	// Get shard
	shard := m.GetShard(key)
	shard.RLock()
...
func (m ConcurrentMap[V]) Remove(key any) {
	// Try to get shard.
	shard := m.GetShard(key)
	shard.Lock()
...
type RemoveCb[V any] func(key any, v V, exists bool) bool

// RemoveCb locks the shard containing the key, retrieves its current value and calls the callback with those params
// If callback returns true and element exists, it will remove it from the map
// Returns the value returned by the callback (even if element was not present in the map)
func (m ConcurrentMap[V]) RemoveCb(key any, cb RemoveCb[V]) bool {
	// Try to get shard.
	shard := m.GetShard(key)
	shard.Lock()
	v, ok := shard.items[key]
...
func (m ConcurrentMap[V]) Pop(key any) (v V, exists bool) {
	// Try to get shard.
	shard := m.GetShard(key)
	shard.Lock()
...
// Items returns all items as map[any]V
func (m ConcurrentMap[V]) Items() map[any]V {
	tmp := make(map[any]V)
...
type IterCb[V any] func(key any, v V)
...
// Keys returns all keys as []any
func (m ConcurrentMap[V]) Keys() []any {
	count := m.Count()
	ch := make(chan any, count)
	go func() {
		// Foreach shard.
		wg := sync.WaitGroup{}
		wg.Add(SHARD_COUNT)
...
func fnv32(key any) uint32 {
	...
	bytes := []byte(fmt.Sprintf("%v", key.(any)))
